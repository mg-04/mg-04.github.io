...
int main(int argc, char **argv) {
	...
	for (;;) {
		...
		n = recvfrom(fd, buf, num_to_recv, 0, (struct sockaddr *) &clnt_un, &clnt_len);

		fprintf(stderr, "\"%s\" received from ", buf);
		write(STDERR_FILENO, clnt_un.sun_path, clnt_len - offsetof(struct sockaddr_un, sun_path));
		fprintf(stderr, " (clnt_len:%d), ", clnt_len);

		// sends back to client
		snprintf(reply, sizeof(reply), "[%d](%s)", ++i, buf);
		n = sendto(fd, reply, strlen(reply), 0, (struct sockaddr *) &clnt_un, clnt_len);
...
int main(int argc, char **argv)
{
	...
	bind(fd, (struct sockaddr *)&my_un, my_len);
